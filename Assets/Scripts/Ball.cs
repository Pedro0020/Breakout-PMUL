...
public class Ball : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        speedMax = initialSpeed * velocityMultiplier*5;
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            if (random < 0.10f)
            {
                ...
                if (u == 0)
                {
                    Instantiate(powerUpPrefab[0], collision.transform.position, Quaternion.identity);
                }
                else
                {
                    Instantiate(powerUpPrefab[1], collision.transform.position, Quaternion.identity);
    ...
    public void ResetBall()
    {
        ...
        transform.position = startPos; // Posición inici    al de la bola
